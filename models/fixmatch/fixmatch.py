...
from utility.retfound_utils import interpolate_pos_embed
...
class FixMatch:
    def __init__(
        ...
        if load_retfound:
            ...
            interpolate_pos_embed(train_model, checkpoint_model)

            # load pre-trained model
            msg = train_model.load_state_dict(checkpoint_model, strict=False)
            print(msg)
            ...
            trunc_normal_(train_model.head.weight, std=2e-5)

            self.train_model = train_model
            self.eval_model = copy.deepcopy(train_model)
    ...
    def _eval_model_update(self):
        ...
        for param_train, param_eval in zip(
            self.train_model.module.parameters(), self.eval_model.parameters()
        ):
            param_eval.copy_(
    ...
    def train(self, args, logger=None):
        ...
        for (x_lb, x_lb_s, y_lb), (x_ulb_w, x_ulb_s, _) in tzip(
            ...
            if self.it == args.num_train_iter:
                self.load_model(
                    os.path.join(
                        os.path.join(args.save_dir, args.save_name), "model_best.pth"
                    )
                )
            ...
            x_lb, x_lb_s, x_ulb_w, x_ulb_s = (
                x_lb.cuda(args.gpu),
                x_lb_s.cuda(args.gpu),
            ...
            with amp_cm():
                ...
                if args.debiased or args.modified_fixmatch:
                    sup_loss = (
                        ...
                        / 2
                        * (
                            ce_loss(logits_x_lb, y_lb, reduction="mean")
                            + ce_loss(logits_x_lb_s, y_lb, reduction="mean")
                        )
                    )
                else:
                ...
                anti_unsup_loss, mask = consistency_loss(
                    logits_x_lb,
                    logits_x_lb_s,
                    "ce",
                    T,
            ...
            if self.it % self.num_eval_iter == 0:
                ...
                self.print_fn(
                    f"{self.it} iteration, USE_EMA: {hasattr(self, 'eval_model')}, {tb_dict}, BEST_EVAL_ACC: {best_eval_acc}, at {best_it} iters"  # noqa: E501
                )
